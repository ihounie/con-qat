...
for SEED in 0 1 2
do
    for EPSILON in 0.00444
    do
        train_id="HIGH_bw_${BW}_eps_${EPSILON}"
        result_dir="./results/$train_id"
        mkdir -p $result_dir

        python -u train_pd_high.py --seed $SEED --model resnet20q --dataset cifar10 --train_split train --lr 0.001 \
